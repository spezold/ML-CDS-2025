...
def get_2d_sincos_pos_embed(embed_dim: int, grid_size: int | tuple[int, int], cls_token: bool = False) -> np.ndarray:
    ...
    :param cls_token: whether to prepend row for class token (True) or not (False)
    :return: ``[int(cls_token) + grid_height * grid_width, embed_dim]``
    """
    grid_size = np.ones(2) * grid_size
