...
from multimods3.utils.video.randerase import RandomErasing
from multimods3.utils.video.functional import crop_clip, normalize, resize_clip, tensor_normalize
from multimods3.utils.video.randaugment import rand_augment_transform
...
class VideoTransform(object):
    ...
    def __init__(self, args: VideoTransformArgs):

        self.training = args.train

        short_side_size = int(args.crop_size * 256 / 224)
        self.eval_transform = Compose([
            Resize(short_side_size, interpolation="bilinear"),
            CenterCrop(size=(args.crop_size, args.crop_size)),
            ClipToTensor(),
            Normalize(mean=args.normalize[0], std=args.normalize[1])
        ])

        self.random_horizontal_flip = args.random_horizontal_flip
        self.random_resize_aspect_ratio = args.random_resize_aspect_ratio
        self.random_resize_scale = args.random_resize_scale
        self.auto_augment = args.auto_augment
        self.motion_shift = args.motion_shift
        self.crop_size = args.crop_size
        self.normalize = torch.tensor(args.normalize)
        ...
        self.spatial_transform = random_resized_crop_with_shift \
            if args.motion_shift else random_resized_crop

        self.reprob = args.reprob
        self.erase_transform = RandomErasing(args.reprob, mode="pixel", max_count=1, num_splits=1, device="cpu")
    ...
    def __call__(self, buffer):
        ...
        buffer = [transforms.ToTensor()(img) for img in buffer]  # [(H W C in [0..255] → C H W in [0., 1.]), ...]
...
class EvalVideoTransform(object):
    ...
    def __init__(
        ...
        self.spatial_resize = Resize(short_side_size, interpolation='bilinear')
